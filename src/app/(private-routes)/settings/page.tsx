...
import { getAuthenticatedUser } from '@/services/api/userService';
...
export default function SettingsPage() {
  ...
  const getUserData = async () => {
    setState('loading');
    try {
      const response = await getAuthenticatedUser(Cookies.get('ACHIRD_TOKEN') as string);
  ...
  if(state === "loading") {
    return <div className="flex flex-col px-8 md:px-16 lg:px-32 py-8">
    ...
    <section className="flex items-center gap-2 mb-8">
      <div className='w-56 h-4 skeleton'></div>
        <button
          className="btn btn-circle btn-sm skeleton"
       ...
       <div className='h-32 w-32 skeleton rounded-full'></div>
        ...
        <h2 className="font-bold text-xl w-32 h-6 skeleton mb-3"></h2>
          ...
          <div className="flex gap-2 flex-wrap items-center justify-center md:justify-start md:items-start">
            <p className="skeleton h-4 w-24 underline underline-offset-2 text-sm font-medium">
              
            </p>
            <p className="skeleton h-4 w-48 underline underline-offset-2 text-sm font-medium">
            </p>
          </div>
          <div className="divider invisible md:visible divider-start m-0 w-12"></div>
    ...
    <section className="mb-8">
      ...
      <form className="flex gap-2 items-center">
        <select className="select select-bordered w-full max-w-xs skeleton">
        </select>
      </form>
  ...
  return (
    <div className="flex flex-col px-8 md:px-16 lg:px-32 py-8">
      ...
      <section className="flex items-center gap-2 mb-8">
        ...
        <div className="tooltip" data-tip="Copiar link">
          <button
            ...
            onClick={() => {
              copy(`${window?.location.origin}/to/${userData?.page_config?.slug}`);
      ...
      <section className="mb-8">
        ...
        <ul className="flex gap-2 flex-wrap">
          {userData?.repositories.map((repository: any)=> (
           <li key={repository.id} className="badge badge-neutral">{repository.name}</li>
          ))}
        </ul>
      </section>
      <section className="mb-8">
      ...
      <section className="mb-8">
        <h2 className="text-lg font-bold mb-2">Configurações</h2>
        <form className="flex gap-2 items-center">
          <select className="select select-bordered w-full max-w-xs" value={userData?.page_config.theme}>
        ...
        <div className="form-control w-64 mt-2">
          <label className="cursor-pointer label">
            <span className="label-text">Página visível ao público</span>
            <input type="checkbox" className="toggle toggle-primary" checked={userData?.page_config.page_is_public} />
          </label>
        </div>
      </section>
    </div>
